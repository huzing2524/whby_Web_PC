<template lang="pug">
  .schedule.w1200.mgauto
    breadcrumb(:breadcrumbList="breadcrumbList")
    .btn_box
      el-button(type="primary" @click="toAdd") 添加班次
    .content
      .title.item
        p 编码
        p 班次名称
        //- p 操作
      .item(v-for="(item, idx) in scheduleList")
        p {{item.uuid}}
        p {{item.name}}
        //- .item_btn_box
        //-   p(@click="btnClick(item, 1)") 修改
        //-   p(@click="btnClick(item, 2)") 删除
    el-dialog(:title="`${isModify?'修改':'添加'}班次`" width="30%" :visible.sync="isShowDialog")
      el-form
        el-form-item(label="班次名称" :label-width="formLabelWidth")
          el-input(v-model="name" autocomplete="off" placeholder="填写班次名称")
