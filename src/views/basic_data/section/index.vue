<template lang="pug">
  .section.w1200.mgauto
    breadcrumb(:breadcrumbList="breadcrumbList")
    .btn_box
      el-button(type="primary" @click="toAdd") 添加工段
    .content
      .title.item
        p 编码
        p 工段名称
        p 操作
      .item(v-for="(item, idx) in sectionList")
        p {{item.id}}
        p {{item.name}}
        .item_btn_box
          p(@click="btnClick(item, 1)") 修改
          p(@click="btnClick(item, 2)") 删除
    el-dialog(:title="`${isModify?'修改':'添加'}工段`" width="30%" :visible.sync="isShowDialog")
      el-form
        el-form-item(label="工段名称" :label-width="formLabelWidth")
          el-input(v-model="name" autocomplete="off" placeholder="填写工段名称")
...
<script>
  ...
  import { SectionMain } from '_api/basic_data'
  export default {
    components: {
      breadcrumb,
    },
