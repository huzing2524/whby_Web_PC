...
<script>
  ...
  import { BatchData, BatchUpload } from '_api/report_import'
  ...
  export default {
    ...
    data() {
      ...
      return {
        breadcrumbList: [
          { name: '生产批次', path: '/report_import/product_patch' },
        ],
        isShowUpload: false,
        year: '2019',
    ...
    methods: {
      ...
      async confirmHandle({ file, date }) {
        ...
        const result = await BatchUpload(data)
      ...
      async getData() {
        ...
        const result = await BatchData(body)
