  ...
  .page.is-scrollable
    ...
    .table
      ...
      .table_item
        .content
          .tip 规格
          input(placeholder="填写规格" v-model="intent.specifications")
        .divider_line
      .table_item
        .content
          .tip_area 产量
          textarea(placeholder="填写产量" v-model="outputArrayString" type="number")
        .divider_line
      .table_item
        .content
          .tip_area 废品 (m³)
          textarea(placeholder="填写废品"
            v-model="scrapArrayString" type="number")
        .divider_line
      .table_item
        .content
          .tip 表层刨花 (T)
          input(placeholder="填写表层刨花"
            v-model="intent.surface_shaving" type="number")
        .divider_line
      .table_item
        .content
          .tip 芯层刨花 (T)
          input(placeholder="填写芯层刨花"
            v-model="intent.core_shaving" type="number")
        .divider_line
      .table_item
        .content
          .tip 表层胶耗 (KG)
          input(placeholder="填写表层胶耗"
            v-model="intent.surface_consumption" type="number")
        .divider_line
      .table_item
        .content
          .tip 芯层胶耗 (KG)
          input(placeholder="填写芯层胶耗"
            v-model="intent.core_consumption" type="number")
        .divider_line
      .table_item
        .content
          .tip 表芯层石蜡 (KG)
          input(placeholder="填写表芯石蜡"
            v-model="intent.watch_core_wax" type="number")
        .divider_line
      .table_item
        .content
          .tip 表芯层固化剂 (KG)
          input(placeholder="填写表芯层固化剂"
            v-model="intent.watch_core_agent")
        .divider_line
      .table_item
        .content
          .tip 表芯层增粘剂 (KG)
          input(placeholder="填写表芯层增粘剂"
            v-model="intent.watch_core_tackifier" type="number")
        .divider_line
      .table_item
        .content
          .tip 表芯层添加剂 (KG)
          input(placeholder="填写表芯层添加剂"
            v-model="intent.watch_core_additive")
        .divider_line
      .table_item
        .content
          .tip 表芯层工艺 (KG)
          input(placeholder="填写表芯层工艺"
            v-model="intent.watch_core_technics")
        .divider_line
      .table_item
        .content
          .tip 脱模剂 (KG)
          input(placeholder="填写脱模剂"
            v-model="intent.parting_agent" type="number")
        .divider_line
      .table_item
        .content
          .tip 审核人
...
<script>
  ...
  import {MaterialCosumptionTable} from '_api/entry_data'
    ...
    data() {
      return {
        ...
        intent:{
          ...
          watch_core_wax: 0,
          watch_core_agent: 0,
          watch_core_tackifier: 0,
          watch_core_additive: 0,
          watch_core_technics: 0,
          parting_agent: 0,
          output:[],// 产量
          scrap:[],// 废品
          remark:"",
    ...
    methods: {
      ...
      checkState() {
        ...
        if(this.outputArrayString === '') {
          console.log(this.outputArrayString)
          alert("产量不能为空")
          return false
        } else {
          return true
      ...
      addData() {
        ...
        body.surface_shaving = parseFloat(this.intent.surface_shaving)
        body.core_shaving = parseFloat(this.intent.core_shaving)
        body.surface_consumption = parseFloat(this.intent.surface_consumption)
        body.core_consumption = parseFloat(this.intent.core_consumption)
        body.watch_core_wax = parseFloat(this.intent.watch_core_wax)
        body.watch_core_agent = this.intent.watch_core_agent
        body.watch_core_tackifier = parseFloat(this.intent.watch_core_tackifier)
        body.watch_core_additive = this.intent.watch_core_additive
        body.watch_core_technics = this.intent.watch_core_technics
        body.parting_agent = parseFloat(this.intent.parting_agent)
        ...
        MaterialCosumptionTable('post', body).then(res => {
          console.log(res)
          if(res.data.res == 0) {
            alert("保存成功")
            this.$router.go(-1)
      ...
      modifyData() {
        ...
        MaterialCosumptionTable('put', body).then(res => {
          console.log(res)
          if(res.data.res == 0) {
            alert("保存成功")
            this.$router.go(-1)
