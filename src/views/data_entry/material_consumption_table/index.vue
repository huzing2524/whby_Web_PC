...
<template lang="pug">
  .page
    ...
    .input-content
      ...
      ExportButton(class="header-button" :fileIds="tableIds" :fileNames="['物料消耗表']")
    .header-content
    ...
    .table-content.el-table--scrollable-y
      el-table(:data="tableData" :id="tableIds[0]" :header-cell-style="headerCellStyle" table-border-color="yellow" :cell-style="cellStyle" @cell-click="onItemClick" :row-class-name="tableRowClassName")
        ...
        el-table-column(prop="scrap" label="废品" flex=1)
          template( slot-scope="scope")
            p(v-for="item,index in scope.row.scrap" :key="index") {{item}}
        el-table-column(prop="surface_shaving" label="表层刨花 (T)" flex=1)
        el-table-column(prop="core_shaving" label="芯层刨花 (T)" flex=1)
        el-table-column(prop="surface_consumption" label="表层胶耗 （KG）" flex=1)
        el-table-column(prop="core_consumption" label="芯层胶耗 （KG）" flex=1)
        el-table-column(prop="watch_core_wax" label="表芯层石蜡 （KG）" flex=1)
        el-table-column(prop="watch_core_agent" label="表芯层固化剂 （KG）" flex=1)
        el-table-column(prop="watch_core_tackifier" label="表芯层增粘剂 （KG）" flex=1)
        el-table-column(prop="watch_core_additive" label="表芯层添加剂 （KG）" flex=1)
        el-table-column(prop="watch_core_technics" label="表芯层工艺水 （KG）" flex=1)
        el-table-column(prop="parting_agent" label="脱模剂 （KG）" flex=1)
  ...
  export default {
        ...
        tableData:[
          {
            ...
            schedule: "B班",// 班次 A
            work_time: "晚",
            specifications:"188",// 这个后台搞错了，要把字段改成 specifications
            surface_shaving: 111,
            core_shaving: 122,
            surface_consumption: 2434,
            core_consumption: 3232,
            watch_core_wax: 1311,
            watch_core_agent: "323",
            watch_core_tackifier: 1141,
            watch_core_additive: "113",
            watch_core_technics: "237",
            parting_agent: 548,
    ...
    methods: {
      ...
      onItemClick(row) {// row就是这一行的数据对象。
        // 这个字段只是画表要用，add_data那边修改数据必须不能多出这个字段。
        delete row.index
        Global.setPressRunBean(row)
        console.log("row.index=" + row.index)
        Global.setScheduleArray(this.scheduleList)
        this.$router.push(`/data_entry/material_consumption_table/add_data?type=modify`)
      },
      ...
      clickAdd() {
        ...
        Global.setScheduleArray(this.scheduleList)
        this.$router.push(`/data_entry/material_consumption_table/add_data?type=add`)
      },
      clickEmport() {
      ...
      initData() {
        ...
        MaterialCosumptionTable('get', body).then(res => {
          if(res.status == 200) {
